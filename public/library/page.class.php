	...
	*/
	class Page {
		...
		function fpage(){
			...
			$fpage = '<nav aria-label="Page navigation example"><ul class="pagination justify-content-center">';
			if(count($arr) < 1)
				$arr = array(4,5,6);
...
//				$str = "&nbsp;<a href='{$this->uri}page=1'>{$this->config["first"]}</a>&nbsp;";
                ...
                $str = "<li class=\"page-item disabled\"><a>{$this->config["prev"]}</a></li>";
            }
				return $str;
		...
		private function pageList(){
			...
			for($i = $inum; $i >= 1; $i--){
				...
				if($page >= 1)
					$linkPage .= "<li class=\"page-item\"><a href='{$this->uri}page={$page}'>{$page}</a></li>";
			}
			/*当前页的信息 */
			if($this->pageNum > 1)
				$linkPage .= "<li class=\"page-item active\"><a href=\"#\">{$this->page}<span class=\"sr-only\">(current)</span></a></li>";
		...
		private function nextlast(){
			...
			if($this->page != $this->pageNum) {
                $str = "<li class=\"page-item\"><a href='{$this->uri}page=" . ($this->page + 1) . "'>{$this->config["next"]}</a></li>";
            }else{
                $str = "<li class=\"page-item disabled\"><a>{$this->config["next"]}</a></li>";
            }
            return $str;
		}
